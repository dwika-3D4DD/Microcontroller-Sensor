...
void loop()
{
  ...
  for (int i = 0; i < 5; i++) {
    dist += sensor.getDistance(); //Calculate the distance in centimeters and store the value in a variable
  }
  mean = dist / 5;
  Serial.print("waktu = "); Serial.println(micros() - lastTime);
  Serial.println( mean ); //Print the value to the serial monitor
  dist = 0;
  delay (100);
}
